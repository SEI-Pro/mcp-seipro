...
class TestWebDocumentosBackend:
    ...
    def test_excluir_documento_delegates_to_web_client(self) -> None:
        result = asyncio.run(
            self.backend.excluir_documento("DOC1", processo="PF", confirmar=True)
        )
        assert result["status"] == "ok"
...
# `linkExcluirDocumento`) grows the contract from 128 to 129 ops; only the web
# backend implements it (no REST/mod-wssei equivalent), so REST's implemented
# count stays at 112 while web's grows from 91 to 92.
