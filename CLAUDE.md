...
- **O gatilho é QUALQUER cor hexadecimal em atributo `style`** — não é o base64, não é o tamanho. Medido ao vivo em 30/07/2026 no doc 2979052 (NT 105). Refutou as hipóteses anteriores: 60 KB de HTML benigno **passa**; 21 KB de texto liso no lugar do brasão **bloqueia**. Primeiro caso isolado: `#333` do rodapé (seção dinâmica 742), em `<hr style="…border-top:medium double #333" />` — trocar por `black` passa; remover `double`, `medium` ou os `;` não resolve. Segundo caso, no dia seguinte: dois `color:#000000` no corpo (seção 740) deixaram o documento inescrevível de novo. É falso-positivo de managed rule sobre o `#` (marcador de comentário SQL) dentro de atributo — vem do template do SEI **e** de conteúdo colado, então não é problema de um documento só
- **Bissecção por prefixo mente; use remoção por janelas.** Truncar prefixo corta no meio de tag/palavra e produz fronteira artificial (num caso o limite caiu em `oportunam|ente`). Remover janelas de mesmo tamanho em offsets diferentes distingue o que interessa: se só uma ou duas remoções fazem passar, há culpado local; se quase todas passam, é score acumulativo. Foi assim que as duas cores `#000000` apareceram. Vale também checar **determinismo** (repetir o mesmo payload 3×) antes de concluir qualquer coisa — evita ler flutuação de score como padrão
- **Correção estrutural** (`sei_editar_secao`): seções com `DinamicaSecaoDocumento == "S"` vão **VAZIAS** no POST, sempre — não como fallback. `EditorRN::montarConteudoSecao` (core do SEI, idêntico em 3.1.7 e 5.0.x) usa `ConteudoOriginal` **lido do banco** para seção dinâmica e nem consulta o que veio no POST. Reenviá-las era mandar ~45% de bytes que o servidor descarta — e era por ali que o gatilho entrava. Critério é `SinDinamica`, **não** `somenteLeitura`: uma seção `somenteLeitura='S'` com `SinDinamica='N'` cai no ramo `else` e grava o que enviamos, então esvaziá-la **apagaria** a seção. Verificação pós-escrita relê e falha alto se alguma dinâmica voltou vazia (o SEI não regenerou)
...
- **Sonda de WAF não-destrutiva** (`_waf_localizar_gatilho`): a chave é que `EditorRN::adicionarVersaoInterno` valida `versao` contra a última do documento e **lança a exceção ANTES de qualquer escrita** (o wssei manda `SinIgnorarNovaVersao='N'`). Logo, um POST com `versao=999999999` atravessa o WAF com o **corpo real** e o SEI o recusa sem tocar no documento. Isso permite bissectar o gatilho em produção sem o risco que inviabilizava a ideia (gravar conteúdo truncado). No 403-WAF a tool esvazia uma seção por vez até o corpo passar, depois bissecta por prefixo dentro da culpada, e reporta seção + posição + trecho. Limitado a `_MAX_SONDAS_WAF=14` e best-effort — diagnóstico nunca mascara o erro original
- **Envio parcial é impossível** — verificado no core: `adicionarVersaoInterno` compara a contagem de seções enviadas com a do banco e lança `Conteúdo do documento incompleto` se vier menos, além de exigir cada `idSecaoModelo` presente. Não existe PATCH de uma seção só; a docstring está certa
...
- **Degrau 2 da escada: cores hexadecimais → `rgb()`.** No 403-WAF, `_converter_cores_hex` reescreve as cores **só dentro do valor de `style`** e refaz o POST; a troca renderiza idêntico. Restringir ao `style` é obrigatório: um regex solto casaria entidades numéricas (`&#233;`) e corromperia o texto. Validado ao vivo — tentativa 1 barrada, tentativa 2 com `rgb()` gravou
- **Âncora SEI: o TEXTO do link tem que ser o `protocoloFormatado` do id.** `EditorRN::processarLinkProtocolo` compara `arrProtocolos[id]` com o texto e, se diferirem, **descarta a tag** deixando só o texto. `idProtocolo` abrange **processos**, não só documentos — `<a id="lnkSei2378553">50300.004460/2024-86</a>` é válido porque 2378553 é o id do processo
- **`validar_referencias=True`** (padrão) parte do TEXTO do link (que é o que o usuário vê) e pergunta ao SEI qual id lhe corresponde. **Não opina sobre âncora de processo** (texto `NNNNN.NNNNNN/AAAA-NN`): a rota de consulta é de documento, e a versão anterior avisava nesses casos por colisão de namespace — chegou a induzir a "correção" de uma âncora correta, que o SEI então descartou e o link sumiu do documento. Avisos vão em `_avisos`, nunca bloqueiam a edição
