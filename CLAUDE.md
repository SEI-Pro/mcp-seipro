...
- **O gatilho NÃO é o base64 nem o tamanho** — medido ao vivo em 30/07/2026 no doc 2979052 (NT 105) por bissecção. Refutou as duas hipóteses anteriores: 60 KB de HTML benigno **passa**; 21 KB de texto liso no lugar do brasão **bloqueia**. O gatilho estava no **rodapé** (seção dinâmica 742, 446 bytes), e o token exato é a **cor hexadecimal `#333`** do `<hr style="…border-top:medium double #333" />`: trocar `#333` por `black` passa; remover `double`, `medium` ou os `;` não resolve. É falso-positivo de managed rule sobre o `#` (marcador de comentário SQL) dentro de atributo — ou seja, **conteúdo do próprio template do SEI**, não do usuário
