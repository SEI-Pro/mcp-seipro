...
Dezenas de tools cobrindo processos, documentos, tramitação, assinatura, blocos, marcadores, acompanhamento, credenciamento, modelos e mais.
...
Tools principais — lista parcial:
...
- `src/todos/server.py` — FastMCP server com dezenas de tools + helpers (`_resolver_documento`, `_resolver_processo`)
...
para o subprocesso que sobe o servidor: `StdioTransport`/`StdioServerParameters`
do SDK MCP usam `env=None` por padrão, que não herda o ambiente do processo
pai. O catálogo de tools carrega normalmente mesmo sem `SEI_URL`/`SEI_WEB_URL`
...
- Praticamente todas as tools funcionam em qualquer SEI 4.0+ sem mod-wssei (web-only)
